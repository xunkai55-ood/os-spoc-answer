...
// the proc
struct proc_struct *the_proc = NULL;
...
do_exit(int error_code) {
    ...
    if (current == the_proc) {
        cprintf("[the_proc] exiting, state is ZOMBIE\n");
    }
	bool intr_flag;
    struct proc_struct *proc;
    local_intr_save(intr_flag);
...
static int
the_main(void *arg) {
    cprintf(">> the proc is running, pid = %d, name = %s\n", current->pid, get_proc_name(current));
    return 0;
}
...
proc_init(void) {
    ...
    // create_the_proc
    cprintf(">> Creating the_proc\n");
    int the_pid = kernel_thread(the_main, NULL, 0);
    if (pid1 <= 0 || pid2 <= 0) {
        panic("create kernel thread init_main1 or 2 failed.\n");
    }
    cprintf(">> the_proc is ready\n");
    the_proc = find_proc(the_pid);
    set_proc_name(the_proc, "the_proc");
}
