...
class fs:
    ...
    # YOUR CODE, 2012013290
        ...
        if inode.getRefCnt() == 1:
            if inode.addr != -1:
                self.dataFree(inode.addr)
            self.inodeFree(inum)
        else:
            inode.decRefCnt()
        # remove from parent directory: delete from parent inum, delete from parent addr
        p_inode = self.inodes[self.nameToInum[self.getParent(tfile)]]
        p_inode.decRefCnt()
        self.data[p_inode.addr].delDirEntry(tfile)
    ...
    # YOUR CODE, 20121011486
        ...
        p_inode = self.inodes[p_inum]
        p_data = self.data[p_inode.addr]
        if p_data.getFreeEntries() <= 0:
            return -1
        if p_data.dirEntryExists(newfile):
            return -1

        # now, find inumber of target
        t_inum = self.nameToInum[target]
        t_inode = self.inodes[t_inum]
        ...
        p_data.addDirEntry(newfile, t_inum)
        tinum = t_inum
    ...
    def createSoftLink(self, target, newfile, parent):
        ...
        if self.data[p_addr].getFreeEntries() <= 0:
            return -1
        if self.data[p_addr].dirEntryExists(newfile):
            return -1
    ...
    # YOUR CODE, 2012011486
        ...
        # find free inode
        f_inum = self.inodeAlloc()
        f_inode = self.inodes[f_inum]

        # if a directory, have to allocate directory block for basic (., ..) info
        addr, ref = (-1, 1)
        if ftype == 'd':
            f_inode.addr = addr = self.dataAlloc()
            f_data = self.data[addr]
            f_data.setType("d")
            f_data.addDirEntry(".", f_inum)
            f_data.addDirEntry("..", p_inum)
            ref = 2

        # now ok to init inode properly
        f_inode.setAll(ftype, addr, ref)

        # inc parent ref count
        p_inode.incRefCnt()

        # and add to directory of parent
        p_data.addDirEntry(newfile, f_inum)
        inum = f_inum
