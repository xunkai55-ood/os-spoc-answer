...
class Bankers(object):
    ...
    def ExecuteProcess(self,index):
        ...
        # YOUR CODE, 2012011486, 2012013290
        for n, a in zip(self.need[index], self.avaliable):
            if n > a:
                return False
        #check END here

        #allocating what they need.
        # YOUR CODE, 20121011486, 2012013290
        for i in range(len(self.RESOURCE)):
            n = self.need[index][i]
            self.avaliable[i] -= n
            self.allocated[index][i] += n
            self.need[index][i] = 0
        #allocating END here
    ...
    def TempSafeCheckAfterRelease(self):
        ...
        # YOUR CODE, 2012011486, 2012013290
        poss = [all(n <= a for n, a in zip(self.need[i], self.avaliable)) for i in range(len(self.need)) if not self.finished[i]]
        return len(poss) == 0 or any(poss)
        #check END here
